...
export const sendFriendRequest = async (receiverId: number) => {
  const response = await axios.post('/api/friends/', { receiver_id: receiverId })
  return response.data
}

export const acceptFriendRequest = async (friendshipId: number) => {
  const response = await axios.post(`/api/friends/${friendshipId}/accept/`)
  return response.data
}

export const declineFriendRequest = async (friendshipId: number) => {
  const response = await axios.post(`/api/friends/${friendshipId}/decline/`)
  return response.data
}
